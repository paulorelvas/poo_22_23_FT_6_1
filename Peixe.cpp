...
int Peixe::nSerie = 500;
double obtemRandom(double min, double max);

Peixe::Peixe(string especie, string cor):especie(especie), cor(cor), numeroSerie(nSerie++){ // construtor do peixe (especie e cor) o peso é sempre 10 no início - depois varia consoante a comida/alimentação
    cout << "Construtor Peixe" << endl;
}
Peixe::Peixe(const Peixe &peixe){      // construtor por cópia
    cout << "Construtor por cópia de Peixe" << endl;

    numeroSerie = peixe.numeroSerie;
    cor = peixe.cor;
    ...
    if (peixe.aquario != nullptr)
        peixe.aquario->adicionaPeixe(this);
}
